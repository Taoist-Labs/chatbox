...
1. 新增联网问答功能。现在托管 AI 服务模型和 Gemini 2.0 flash(API) 可以参考互联网的实时信息进行回答，并在消息中列出参考信息来源
2. 新增查看消息生成时的首字延迟时间（first-token latency），可在设置中开启
3. 新增发送图片的预处理能力。现在你可以发送更多格式的图片（例如 svg、gif），应用会自动转化成模型可以接收的图片格式，并且自动调节图片大小以符合模型要求
4. 双击话题名称可以快速弹出话题列表
...
1. 现在你可以发送网页链接。应用将自动获取网页内容并将其包含在聊天上下文中。适用于所有模型。
2. 现在你可以向任何模型发送文本文件。应用将在本地解析文件内容并将其包含在聊天上下文中。
...
3. 现在你从应用复制的文本，将会以纯文本的形式复制到剪贴板（不再包含背景颜色，这个长期存在的小bug终于修复了）
...
- 新增图片生成功能（Image Creator)，现在你可以在应用中生成图片了。由 Dall-E-3 模型提供支持。
- 优化一些交互问题
...
- 内置 AI 服务“托管 AI 服务”，开箱即用，直接高速访问，再也不需要折腾网络、账户和各种技术术语。
- 修复了重新启动时夜间主题失效的问题
- 修复回答生成时无法切换会话的问题
- 修复消息编辑时的卡顿问题
